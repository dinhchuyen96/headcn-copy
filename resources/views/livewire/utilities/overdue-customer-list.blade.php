<div>
    <div>
        <div class="page-content fade-in-up">
            <div class="ibox">
                <div class="ibox-head">
                    <div class="ibox-title">Danh sách khách hàng nợ quá hạn</div>
                </div>
                <div class="ibox-body">
                    <form>
                        <div class="form-group row">
                            <label for="ClaimCode" class="col-1 col-form-label">Mã khách hàng</label>
                            <div class="col-3">
                                <input id="searchCode" name="searchCode" type="text" class="form-control"
                                    wire:model.debounce.1000ms="searchCode">
                            </div>
                            <label for="ClaimCode" class="col-1 col-form-label">Tên khách hàng</label>
                            <div class="col-3">
                                <input id="searchName" name="searchName" type="text" class="form-control"
                                    wire:model.debounce.1000ms="searchName">
                            </div>
                            <label for="ClaimCode" class="col-1 col-form-label">Số điện thoại</label>
                            <div class="col-3">
                                <input id="searchPhone" name="searchPhone" type="text" class="form-control"
                                    ...
                                    <thead>
                                        <tr role="row">
                                            ...
                                            <th class="{{ $key_name == 'sr' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                                ...
                                                style="width: 164.5px;" wire:click="sorting('sr')">Mã khách hàng</th>
                                            <th class="{{ $key_name == 'sr_created_date_time' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                                tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                                style="width: 164.5px;" wire:click="sorting('sr_created_date_time')">Tên
                                                tên hàng</th>
                                            <th class="{{ $key_name == 'sr_closed_date_time' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                                tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                                style="width: 164.5px;" wire:click="sorting('sr_closed_date_time')">Số
                                                điện thoại</th>
                                            <th class="{{ $key_name == 'sr_closed_date_time' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                                tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                                style="width: 164.5px;" wire:click="sorting('sr_closed_date_time')">Tổng
                                                dư nợ</th>
                                        </tr>
                                    </thead>
                                    ...
                                    <tbody>
                                        ...
                                        @forelse ($data as $value)
                                            <tr data-parent="" data-index="1" role="row" class="odd">
                                                ...
                                                <td>{{ $value->name ?? '' }}</td>
                                                <td>{{ $value->phone ?? '' }}</td>
                                                <td>{{ $value->remainAmount ? number_format($value->remainAmount ?? 0) : number_format($value->sum_total_money ?? 0) }}
                                                    VNĐ</td>
                                            </tr>
                                        @empty
                                            <tr>
                                                <td colspan="12" class="text-center text-danger">Không có bản ghi nào.
                                                </td>
                                            </tr>
                                        @endforelse
                                    </tbody>
