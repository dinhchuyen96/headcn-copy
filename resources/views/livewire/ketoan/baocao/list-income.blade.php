<div>
    <div class="page-content fade-in-up">
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">DANH SÁCH PHIẾU THU</div>
            </div>
           <div class="ibox-body">
                <form>
                    <div class="form-group row">
                        ...
                        <label for="CustomerAddress" class="col-1 col-form-label ">Loại phiếu thu</label>
                        <div class="col-3">
                            <select wire:model="incometype" id="incometype"
                            class="custom-select select2-box col-6" multiple size=11>
                            <option value="">--Tất cả--</option>
                                ...
                                <option value="5">Dịch vụ KTĐK</option>
                                <option value="6">Dịch vụ sửa chữa</option>
                                <option value="7">Nợ tồn</option>
                            ...
                            <table class="table table-striped table-bordered dataTable no-footer"
                                ...
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="{{ $key_name == 'code' ? ($sortingName == 'desc' ? 'sorting_desc w-20 p-3' : 'sorting_asc w-20 p-3') : 'sorting w-20 p-3' }}"
                                            tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                            wire:click='sorting("code")'>Ngày hạch toán</th>
                                        <th class="{{ $key_name == 'customer' ? ($sortingName == 'desc' ? 'sorting_desc w-25 p-3' : 'sorting_asc w-25 p-3') : 'sorting w-25 p-3' }}"
                                            tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                            wire:click='sorting("name")'>Người nộp/KH</th>
                                        <th class="{{ $key_name == 'name' ? ($sortingName == 'desc' ? 'sorting_desc w-25 p-3' : 'sorting_asc w-25 p-3') : 'sorting w-25 p-3' }}"
                                            tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                        ...
                                        <th class="{{ $key_name == 'address' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                            tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                            wire:click='sorting("address")'>Mã TK Nộp</th>
                                        <th class="{{ $key_name == 'address' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                            tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                            wire:click='sorting("address")'>Số TK Nộp</th>
                                        <th tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                            class="{{ $key_name == 'total_money1' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                            wire:click='sorting("total_money1")'>Mã TK Nhận</th>
                                        <th tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                            class="{{ $key_name == 'total_money1' ? ($sortingName == 'desc' ? 'sorting_desc' : 'sorting_asc') : 'sorting' }}"
                                            wire:click='sorting("total_money1")'>Số TK Nhận</th>
                                        <th tabindex="0" aria-controls="category-table" rowspan="1" colspan="1"
                                ...
                                <tbody>
                                    @forelse ($data as $key => $value)
                                        ...
                                        <tr data-parent="" data-index="1" role="row" class="odd">
                                            ...
                                            <td>{{ isset($value->customer_name) ? $value->customer_name : '' }}</td>
                                            <td>{{ $value->note }}</td>
                                            <td>{{ isset($value->type) ? $this->getOrderReceiptType($value->type) : 'Thu tiền hàng' }}
                                            </td>
                                            <td>{{ $value->from_account_code }}</td>
                                            <td>{{ $value->from_account_number }}</td>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="receivedate" class="col-12 col-form-label">Ngày hạch toán <span
                        ...
                        <div class='col-sm-12'>
                            <select wire:model.lazy="from_account_money_id" id="from_account_money_id"
                                ...
                                @foreach ($this->account_money_list as $item)
                                    <option {{ $this->from_account_money_id == $item->id ? 'selected' : '' }}
                                        value="{{ $item->id }}">
                                        {{ $item->account_code .
                    ...
                    <div class="form-group row">
                        <div class='col-sm-12'>
                            <select wire:model="to_account_money_id" id="to_account_money_id"
                                class="custom-select select2-box col-sm-12">
