<div>
    <div class="page-content fade-in-up" style='padding-top:0px !important' >
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">Bán buôn phụ tùng</div>
            </div>
            <div class="ibox-body">
                <div class="form-group row mt-1">
                    <label for="name" class="col-1 col-form-label pr-0">Mã khách hàng<span class="text-danger"> *</span></label>
                    <div class="col-3">
                        <input id="name" wire:model.lazy="code" placeholder="Mã khách hàng" type="text"
                    ...
                    <div class="col-3">
                        <input  wire:model.lazy="phone" placeholder="Số điện thoại" type="number"
                               class="form-control form-red" required="required" {{$status?'disabled':''}}>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-3">
                        <select {{$status || $order_id ?'disabled':''}}  class="form-control select2-box" id='selectwarehouse'
                        wire:model.lazy='selectwarehouse'>
                        @if (!empty($warehouses))
                ...
                <div class="form-group row justify-content-end mt-4" style="padding: 0 20px;">
                    <button  @if ($status) style='display:none' @endif type="button" wire:click="addBarCode('{{$barcode}}')" class="btn btn-primary add-new">
                        <i class="fa fa-plus"></i> THÊM
                    </button>
                    <div class="col-md-1">
                        <button @if ($status) style='display:none;' @endif
                        type="button" class="btn btn-info add-new" data-toggle="modal"
                        data-target="#modal-form-import"><i
                                class="fa fa-upload"></i> IMPORT
                        </button>
                    </div>
                    <div class="text-right virtual ml-1">
                        <input type='checkbox' id='chkIsVirtual'
                 ...
                 @livewire('component.list-input-part-wholesale',['type'=>1])
    ...
    <script>
        ...
        window.addEventListener('hello', event => {
            let titleMessage = 
            'Thêm mới thành công. Bạn có muốn in phiếu bán buôn phụ tùng bằng pdf không?';
            Swal.fire({
                title: titleMessage,
                icon: 'success',
