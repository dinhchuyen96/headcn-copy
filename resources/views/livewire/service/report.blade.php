<div>
    <div class="page-content fade-in-up">
        <div class="ibox">
            <div class="ibox-head">
                <div class="ibox-title">Báo cáo kiểm tra định kỳ</div>
            </div>
            <div class="ibox-body">
                <form>
                        ...
                        <div class="col-sm-12 table-responsive">
                            <table class="table table-bordered dataTable no-footer"
                                ...
                                @forelse ($data as $key => $item)
                                    @foreach($item->periodicChecklists as $i => $periodic)
                                        <tr data-parent="" data-index="1" role="row" class="odd">
                                            @if (!$i)
                                            <td rowspan="{{ $item->periodicChecklists->count() + 1 }}">{{ $item->name }}<br/> ID: {{ $item->username }}</td>
                                            @endif
                                            <td>{{ $periodic->created_at->format('d/m/Y') }}</td>
                                            <td>{{ @$periodic->content_request }}</td>
                                            <td>{{ @$periodic->motorbike->motor_numbers }}</td>
                                            <td>{{ @$periodic->motorbike->model_code }}</td>
                                            <td>{{ number_format(@$periodic->order->total_money) }}</td>
                                        </tr>
                                        @if ($i == $item->periodicChecklists->count() - 1)
                                            <tr data-parent="" data-index="1" role="row" class="odd">
                                                <td colspan="5" class="text-right font-weight-bold">{{ number_format(@$item->periodicChecklists->sum('order.total_money')) }}</td>
                                            </tr>
                                        @endif
                                    @endforeach
                                @empty
