...
class ThuDetailExport implements FromCollection, WithHeadings, WithMapping, WithEvents, WithCustomStartCell
{
    ...
    function __construct($fromDateBefore, $toDateAfter, $customerPhone, $chassicNo, $engineNo, $motorNumbers, $fromDate, $toDate, $key_name, $sortingName, $key_name2, $sortingName2)
    {
        $this->fromDateBefore = trim($fromDateBefore);
        $this->toDateAfter = trim($toDateAfter);
        $this->customerPhone = trim($customerPhone);
        $this->chassicNo = trim($chassicNo);
        $this->engineNo = trim($engineNo);
        $this->motorNumbers = trim($motorNumbers);
            ...
            AfterSheet::class => function (AfterSheet $event) {
                ...
                foreach ($this->userOrders as $key => $value) {
                    if ($value->type_table == 3) {
                        $active_sheet->getStyle('A' . ($key + 5) . ':I' . ($key + 5))->applyFromArray($default_font_style1);
                    } elseif ($value->type_table == 0) {
                        $active_sheet->getStyle('A' . ($key + 5) . ':I' . ($key + 5))->getFont()->getColor()->setARGB('E74C3C');
                    }
                }
