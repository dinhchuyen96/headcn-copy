...
class MoneyInModel implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithEvents, WithTitle
{
    ...
    {
        return 'Tổng giá trị xe theo model';
    }
    ...
    public function collection()
    {
        ...
        //     ->whereHas('warehouse')->where("motorbikes.sell_date", null)->groupBy('motorbikes.warehouse_id')->select(DB::raw('sum(motorbikes.price) as totalMoney'), 'motorbikes.warehouse_id');
        $query = Motorbike::leftJoin('warehouse', function ($query) use ($toDate, $fromDate) {
        ...
        })->groupBy('motorbikes.model_code')->select(DB::raw('sum(motorbikes.price) as totalMoney'), 'motorbikes.model_code', DB::raw('count(motorbikes.id) as totalItem'));

        if ($this->key_name) {
            $query->orderBy($this->key_name, $this->sortingName);
        }
        $this->totalMoney = $query->sum("motorbikes.price");
        $data = $query->get();
        return $data;
    }
