...
class BaoCao extends BaseLive
{
    ...
    public function render()
    {
        ...
        $data = $query->orderBy($this->key_name, $this->sortingName)->paginate($this->perPage)->setPath(route('dichvu.baocao.sctt.index'));
        return view('livewire.service.baocao', compact('data'));
    }
    ...
    public function getQuery()
    {
        ...
        $toDate = $this->toDate;
        $query = User::with(['repairBills' => function ($q) use ($fromDate, $toDate) {
            if ($fromDate) $q->where('created_at', '>=', $fromDate);
            if ($toDate) $q->where('created_at', '<=', $toDate);
            $q->orderByDesc('id');
        }, 'repairBills.motorbike', 'repairBills.order'])
            ->whereHas('repairBills.order')
            ->whereHas('repairBills.motorbike')
