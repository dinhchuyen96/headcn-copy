...
use App\Exports\ListIncomeExport;

use Livewire\Component;

class ListIncome extends BaseLive
{
    public $totalmoney = 0;
    ...
    protected $listeners = ['setfromDate', 'settoDate','settransfer_date'];
    ...
    public function render()
    {
        ...
        ->withPath(route('ketoan.baocao.dsthu.index'));
        return view('livewire.ketoan.baocao.list-income',['data'=>$data]);
    }
    ...
    public function GetIncomes(){
        $query = DB::table('receipts as v1');
        $query->leftJoin('account_money as v2','v1.account_money_id','v2.id');
        $query->leftJoin('account_money as v3','v1.from_account_money_id','v3.id');
        $query->leftJoin('customers as v4','v1.customer_id','v4.id');

        if(isset($this->searchfromDate)){
            $query->whereDate('v1.receipt_date','>=',$this->searchfromDate);
        }
        if(isset($this->searchtoDate)){
            $query->whereDate('v1.receipt_date','<=',$this->searchtoDate);
        }
        if(isset($this->content)){
            $query->where('v1.note','like',$this->content.'%');
        }
        if(isset($this->incometype) && count($this->incometype) >0){
            $query->whereIn('v1.type',$this->incometype);
        }
        $query->orderBy('v1.receipt_date','DESC');
        $query->select('v1.id','v1.money','v1.receipt_date','v1.type',
        'v1.created_at','v1.note',
        'v2.account_code','v2.account_number',
        'v3.account_code as from_account_code','v3.account_number as from_account_number','v4.name as customer_name');
    ...
    public function export()
    {
        ...
        if ($data->count() == 0) {
            ...
            $this->emit('close-modal-delete');
            return Excel::download(new ListIncomeExport($data), 'dsthu_' . date('Y-m-d-His') . '.xlsx');
        }
    }
