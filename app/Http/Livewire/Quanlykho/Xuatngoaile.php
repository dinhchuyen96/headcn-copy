...
class Xuatngoaile extends BaseLive
{
    public $warehouses;
    ...
    public function addAccessory($i)
    {
        $this->validate(
            ...
            [
                ...
                'warehouses_name.required' => 'Bắt buộc chọn kho xuất',
            ]
        );
        ...
        if ($this->accessory_quantity > $this->accessory_quantity_current){
            $this->dispatchBrowserEvent('show-toast', ['type' => 'error', 'message' => 'Số lượng phụ tùng trong kho chỉ còn '.$this->accessory_quantity_current]);
            return;
        }
    ...
    {
        $accessory = Accessory::where('code', $code);
        if ($this->warehouses) {
            $accessory = $accessory->where('warehouse_id', $this->warehouses);
        }
        if ($this->position_in_warehouse){
            $accessory = $accessory->where('position_in_warehouse_id', $this->position_in_warehouse);
        }
        ...
        if(!empty($accessory)){
            ...
            $this->changeWarehouses($accessory->warehouse_id);
            $this->changePosition($accessory->position_in_warehouse_id);
            $this->readOnly = true;

        } else {
            $this->dispatchBrowserEvent('show-toast', ['type' => 'error', 'message' => 'Kho này không có phụ tùng trên']);
            $this->accessory_code = '';
            $this->accessory_name = '';
            $this->readOnly = false;
        }
    ...
    {
        $position = PositionInWarehouse::where('id', $id)->first();
        if(!empty($position)){
            $this->position_in_warehouse = $position->id;
    ...
    public function render()
    {
        ...
        return view('livewire.quanlykho.xuatngoaile', [
    ...
    public function store()
    {
        ...
        if (!empty($this->accessories)) {
            try {
                ...
                DB::commit();
                $this->accessories = [];
                $this->dispatchBrowserEvent('show-toast', ['type' => 'success', 'message' => 'Xuất ngoại lệ thành công']);
            } catch (\Throwable $th) {
                DB::rollBack();
                $this->dispatchBrowserEvent('show-toast', ['type' => 'error', 'message' => 'Thêm mới không thành công']);
                $this->resetInput();
